...
구현 커밋: `9425751` (`fix: preserve preferences and legacy tea adjustments`). 기능 브랜치 `codex/tw-01-preferences`에서 구현했다. 통합 전 완료 조건을 확인했으며 병합·통합 후 결과는 아래에 기록한다.
...
브라우저 harness·주입 서버·원본 캡처·검증 로그는 Git 외부의 이번 작업 산출물 `tw-01` 폴더에 보관한다. 화면/저장 검증은 데스크톱 브라우저의 결과이며 실제 iOS/Android·설치형 PWA·오디오 출력·스크린리더는 미검증이다. 기존 설치의 v57→새 버전 업데이트와 외부 글꼴 오프라인 조합은 TW-07·08에서 다시 확인한다. 다중 탭 동일 필드의 동시 쓰기와 구버전 앱의 쓰기를 무손실 병합한다고 보장하지 않는다.
