...
- **TW-06 번체 구현 완료·로컬 검증 완료**: `scripts/tags.js` → `preferences.js` → `i18n.js` → `locales/ko*.js` → `locales/zh-TW*.js` 순으로 로드한다. 번체 공통·설정·도움말 카탈로그, 언어별 기본 15개와 기존·사용자 원문 고정 태그, SW v63을 기능 브랜치에 구현했고 Node 계약 검증 61개가 통과했다. `tests/localization.html`, `tests/traditional-chinese.html`, `tests/font-preflight.html`, `tests/offline-assets.html`은 전용 8134에서 실행한다. 로컬 검증 결과는 [TW-06 기록](docs/CHANGELOG.md#tw-06-traditional-chinese)을 따른다. 통합·배포는 아직 완료하지 않았다. 상세 계약은 [언어별 기본 단어 목록](docs/design-localization.md#locale-default-vocabulary)을 따른다. TW-07 A안과 TW-08 실기기/PWA 검증이 남으며, 현지인 검수는 배포 후 진행한다.
