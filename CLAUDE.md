...
- **우림 연출 v0.2 대부분 적용됨**(활성 기준 [DESIGN.md](DESIGN.md)): 추출 포화 색곡선·찻물 세로 그라데이션·찻잎 sway/시차 펼침/pour 난류·따뜻한 앰버 앰비언트 글로우·김 강화(블러)+**주전자 뚜껑·꼭지 다포트 김**(`steamPorts`, 다구별 실제 증기 경로에 맞춰 배치)·수면 일렁임·임박 SVG throb·pour 물줄기/잔물결·둥근 도구별 링 중심(`ringGeom`)·**차종별 7가지 잎 형태**(`leafProfile`)·**idle 상태 마른 잎 표시**(유리 도구). 단, **개완 뚜껑 열림은 아직 미작동**(`#g-lid`가 숨겨짐).
- **반복 사용 상태 영속 완료**: `teaTimer.preferences.v1`에 차·다구·차별 권장 대비 시간 보정값·음소거를 저장한다. 회차와 진행 중 타이머는 복원하지 않으며, 도움말에서 저장값을 지울 수 있다.
