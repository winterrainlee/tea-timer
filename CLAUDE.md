...
- **우림 연출 v0.2 적용됨**(근거 [design-v0.2.md](design-v0.2.md)): 추출 포화 색곡선·찻물 세로 그라데이션·찻잎 sway/시차 펼침/pour 난류·개완 뚜껑 열림·따뜻한 앰버 앰비언트 글로우·김 강화(블러)+**주전자 뚜껑·꼭지 다포트 김**(`steamPorts`)·수면 일렁임·임박 SVG throb·pour 물줄기/잔물결·둥근 도구별 링 중심(`ringGeom`).
- **TODO**: 선택값 **localStorage 영속**, 앱 **아이콘 PNG**(현재 manifest가 가리키는 `icons/*.png` 없음), **폰트 셀프호스팅**(완전 오프라인), 백그라운드 타이머 보정·오디오 클록 예약.
