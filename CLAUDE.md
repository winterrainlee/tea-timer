...
- **TW-06 번체 구현·로컬 검증·통합 완료**: `scripts/tags.js` → `preferences.js` → `i18n.js` → `locales/ko*.js` → `locales/zh-TW*.js` → `locales/zh-CN*.js` 순으로 로드한다. UI 문구는 번역하고 언어별 기본 15개·기존 builtin·사용자 입력은 원문을 유지한다. `tests/localization.html`, `tests/traditional-chinese.html`, `tests/simplified-chinese.html`, `tests/font-preflight.html`, `tests/offline-assets.html`은 전용 8134에서 실행한다. Node 계약 검증은 `node --test tests/*.test.cjs`. [검증·통합 기록](docs/CHANGELOG.md#tw-06-traditional-chinese)과 [독립 기본 목록 계약](docs/design-localization.md#locale-default-vocabulary)을 따른다. [간체 확장 기록](docs/CHANGELOG.md#simplified-chinese-local)도 함께 확인한다. TW-07 A안은 구현·로컬 검증했고 실기기/PWA·배포는 남으며 현지인 문구 검수는 배포 후 진행한다.
