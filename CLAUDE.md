...
- **TODO**: 선택값 **localStorage 영속**, 앱 **아이콘 PNG**(현재 manifest가 가리키는 `icons/*.png` 없음), **폰트 셀프호스팅**(완전 오프라인), 백그라운드 타이머 보정·오디오 클록 예약, 동양식 다호/티팟 임박 연출 정교화, 둥근 도구별 링 중심 미세조정(동양식 다호·티팟이 약간 높음).
