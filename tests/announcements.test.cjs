...
test('current release shows the five agreed items without draft or empty past sections', () => {
  ...
  assert.deepEqual(sections[0].items.map(item => item.id), ['main-ui', 'feeling-words', 'chinese-locales', 'creator-feedback', 'offline-usage']);
